...
- **官方市场日志规范对齐**：
  - 全链路移除内置 `logging` 模块直接创建的 stdlib logger（`logging.getLogger`）与未使用的导入及回退分支。
  - 统一接入框架官方 `from astrbot.api import logger`，确保所有模块的日志格式、颜色与分级归档均受 AstrBot 统一管控，满足官方插件市场准入规范。
...
- **统一媒体管线与多清晰度候选池**：
  - 彻底打通 FxTwitter、Nitter HTML 及被动推文链接的视频变体候选池，完整保留 4K、1080p、720p、480p 等各档清晰度选项与音视频流元数据（分辨率、比特率、时长），杜绝提前硬编码或丢失候选。
  - Nitter RSS 媒体解析优先走内部 `status_resolve` 极速获取推特官方 CDN 直链与完整变体池，大幅降低对不可控外部第三方 `xdown.app` 服务的依赖。
- **推图指令视频专线与特许放行**：
  - 指令 `/推图` 语法扩展，支持过滤类型参数：`/推图 用户名 [数量] [视频/图片]` 与 `/推图 用户名 [视频/图片]`。
  - 显式指定视频时，上游结合多页翻页专线（FxTwitter 游标跟随 / Nitter 本地过滤）搜集视频推文，发送层局部特许放行视频下载与发送（`force_media=True`），零全局状态污染。
  - 支持根据指令参数自适应展示查询进度文案（「相册图片推文」「视频推文」「相册媒体推文」）。
- **极简纯净模式（`send_batch_summary_enabled` 语义升级）**：
  - 关闭 `send_batch_summary_enabled` 后开启全局极简纯净模式：全链路（定时检查、轮询间隔、手动命令）不再发送任何批次横幅与合并统计头节点，推文末尾的 📎 附件统计行也一并省略，仅呈现推文核心正文与媒体。
- **Dashboard「插件配置」批量草稿保存与吸顶优化**：
  - WebUI「插件配置」改为一次性校验并批量提交保存全部配置草稿，保存成功后精准热重载当前插件；热重载失败时安全保留已存配置并给出明确提示。
  - 配置操作条与分组导航栏根据顶栏实际高度通过 ResizeObserver 动态计算 sticky 偏移，窗口缩放或小屏下保持平滑吸顶且不遮挡内容。
- **Dashboard「实例能力诊断」按需选测与 UI 规范化**：
  - 彻底解绑测试目标：测试用户名、搜索标签/关键词、List ID 相互独立，按需选测；留空项目自动跳过，无需等待无关项目的网络请求，大幅缩短诊断等待耗时。
  - 规范表单文案与占位提示，增加全空防呆校验 Toast；诊断结果卡片自适应仅渲染实际执行了的探测项目与可用状态。

### Changed & Improved

- **风控二分排雷提速与日志降噪**：
  - OneBot/NapCat 合并转发遭遇腾讯平台内容风控（`retcode 1200 / res_id 校验失败`）时，跳过无意义的同包底层重复重试，毫秒级直接进入二分排雷或拆分逻辑，大幅削减发送阻塞耗时。
  - 全面脱敏风控警告与拒收详情日志，消除敏感会话目标、消息内容与几十位密文哈希视觉污染。
  - 多分段合并转发拒收状态严格隔离，当关闭纯文本降级时，直接输出友好的风控略过提示（「⚠️ 内容触发平台风控，已自动略过」），杜绝违规内容纯文本刷屏。
- **自建实例地址全面脱敏**：
  - 定时检查摘要「失败」行、失败详情、后台日志与错误汇总不再泄露自建 Nitter 实例的 IP 与域名地址：RSS 重试与 HTML 轮换全面采用序号标签（`#1`、`#2`），生效实例与轮换轨迹统一标识为 `Nitter` / `FxTwitter`。
  - 新增聚合层兜底 `redact_instance_urls`，确保摘要与错误汇总中的残余裸 URL 一律替换为「实例地址」，防止新增错误路径再泄露私有地址。
  - 修正多轮重试失败计数文案，准确展示尝试实例数与累计请求轮次。
  - 任务摘要日志 `safe_task_log` 边界对生效实例、轮换轨迹与失败详情强制执行 `redact_instance_urls` 保护；HTML 实例池轮换失败日志全面接入 `redact_instance_urls` 与 `sanitize_sensitive_text` 双重脱敏。
  - 定时检查会话消息 `format_message` 的失败源与基线重建失败详情对齐 `sanitize_diagnostic` 脱敏标准。
- **视频节点纯媒体化**：
  - 合并转发与逐条推送中的视频/GIF 独立节点不再附加重复的「视频/GIF 附件」文本说明节点，直接发送纯视频，消除冗余占位。
- **外部链接标准化与引用清理**：
  - 扩展外部链接识别规则，全面覆盖 `localhost`、IPv4 地址及通用端口号。
  - 自动识别自建 Nitter 引用推文链接（如 `http://localhost/username/status/123#m`）并规范化转换为 `https://x.com/...` 链接。
  - 剥离外部链接后自动清理残留的孤立引用破折号行（如 `^—\s*$`），避免渲染空白破折号。
- **FxTwitter 媒体抓取翻页与 Overfetch**：
  - `fetch_user_timeline` 实现受 `html_max_pages` 深度控制的游标跟随翻页循环，在纯文本过滤或转推过滤导致候选不足时自动向后翻页直至满足请求数量或游标耗尽。
  - 开启纯文本过滤时首屏请求量动态上浮（Overfetch），消除小数量拉取时的 upstream HTTP 500 报错并扩大媒体候选窗口。
- **配置与文档描述精简**：
  - 精简澄清 `filter_plain_text_enabled`（只推包含图片/视频推文）与 `media_only_enabled`（只发媒体，去除正文文本与翻译）的配置语义。
  - `_conf_schema.json` 与文档同步更新：明确 `html_max_pages` 对 FxTwitter 媒体翻页深度的控制，标注手动 `/推文` 与 `/推图` 同样遵循 `filter_reposts_enabled` 全局总开关。
  - 同步更新 `advanced.md` 与 `README.md` 中的手动命令转发过滤说明，明确手动 `/推文` 与 `/推图` 遵循全局开关，`/镜像测试` 始终保留转发，`/推文搜索` 固定过滤纯转推。
  - 统一调度任务审计日志中的触发原因文案（如 `手动检查`、`手动命令 (推图)`）。

### Fixed

- **Nitter 手动推图与视频模式媒体预装配**：
  - 修复通过 Nitter 获取推文时，因初始媒体信息未装配导致在视频模式或图片模式下推文被全部误过滤清空的问题：过滤前强制调用媒体服务装配媒体元数据（包含 `force_all_media=is_video_mode`）。
- **大视频超出上限自适应借档降级**：
  - 修复视频在超出 `media_max_size_mb` 时无法自动降级而被整条跳过的缺陷；当选中画质超限时，自动向下借档选取次高分辨率文件顺畅发送（例如 4K 超限自动降级为 1080p）。
- **图片媒体直链画质对齐**：
  - 修复部分链路图片直链未按 `media_quality` 统一的问题，确保 HTML 抓取、FxTwitter/被动链接解析及 xdown 兜底统一按 high/medium/low 规则对齐。
- **合并转发去视频降级与节点信封规范化**：
  - 修复合并转发连续失败后的去视频降级链路因参数缺失中断的问题，确保去视频重发、二分拆分与直发降级全链路畅通。
  - OneBot raw 直发节点信封统一对齐 OneBot v11 标准字段名（`user_id` / `nickname`），全面兼容 NapCat 与 LLOneBot。
...
> **💡 1.5.0 运维与配置建议**：
> 得益于本次升级的「博主合并 RSS」与「List RSS 优先」管道，后台定时检查对自建 Nitter 实例的请求压力大幅降低。
>
> 若自建实例运行稳定且无 429 限流风险，建议在设置中将全局检查间隔（`check_interval_minutes`，默认 30 分钟）适当调低至 **15~20 分钟**。更短的检查间隔不仅能大幅提升推文推送的实时性，还能使每轮检查的增量推文更平缓（极高概率首屏瞬间命中水位），进一步减少深翻页负担。
...
- WebUI 面板大修：删除约 1400 行死 CSS（"Premium Design Concept" 遗留层），style.css 从 2942 行精简到约 1000 行；合并「系统维护清理」tab 到「控制台总览」底部 danger zone（5 tab → 4 tab，后新增「插件配置」tab 共 5 tab）；总览新增已配置实例列表展示。
- WebUI 面板体验打磨：修复暗色主题下 danger zone 白底白字；刷新/切换分组有未保存修改时先确认再丢弃；原生 confirm/alert 收编进自带弹窗与内联校验；推送历史新增送达状态筛选（后端 `web/history` 支持 `status` 参数）与「含媒体」徽章改用后端记录字段判定；重推支持选择目标（已下线目标不可选）；历史时间改相对显示（悬浮看完整时间）；每日定时时间非法输入内联提示；总览指标卡紧凑化 + 等宽数字；实例探测防重复提交；弹窗补焦点循环。
- 维护区显示数据存量：danger zone 展示媒体缓存文件数与体积、Seen 记录总数及分组计数（随 `web/overview` 返回），清理/重置前可见规模。
- 总览提示修复与增强：移除无异常时「1 项」假提示；新增「分组已停用」提示（可点击跳转到对应分组）；warning 级别正确渲染为黄色；指标卡（除调度器外）点击跳转分组管理。
- 原生 select 下拉替换为自绘组件（历史分组/状态、Seen 分组），样式与主题统一，支持键盘导航与点击外部关闭；分组列表切换补过渡动画；历史工具栏间距整理、筛选按钮主色、条数输入加宽修复数字裁切。
- 分组订阅管理补齐：补回面板重写期间遗漏的目标作者黑名单管理入口与订阅批量导入/移除（`web/subscriptions/import` / `web/subscriptions/delete`）；分组与订阅源的添加、移除等变更改为确认弹窗并随确认自动保存（「添加并保存」「移除并保存」）；移除分组编辑器「检查」按钮与 `web/check` 端点。

### Fixed

- 修复操作状态横幅空闲时残留「空转圈」的问题：`hidden` 属性被显式 `display: flex` 击穿，补 `.action-status[hidden]` 规则。
