...
- **风控二分排雷提速与日志降噪**：
  - 合并转发遭遇腾讯内容风控（`retcode 1200 / res_id 校验失败`）时，精准跳过无意义的同包底层重复重试，直接就地启动二分排雷，排雷耗时大幅缩减。
  - 保留 `WARN` 级别警示可见度，对控制台输出进行脱敏与格式整形，将冗长乱码堆栈收进 `DEBUG`，彻底消除大段长哈希字符视觉污染。
- **推图指令视频专线与特许放行**：
  - 指令 `/推图` 扩展支持类型参数：`/推图 用户名 [数量] [视频/图片]` 与 `/推图 用户名 [视频/图片]`。
  - 显式指定视频（`视频` / `video`）时，上游抓取层结合多页翻页专线搜集视频推文，发送层局部特许放行视频下载与发送（`force_media=True`），零全局状态污染，受单媒体大小与最长时长配置保护。
  - 未指定时维持原有纯相册图片墙行为。
