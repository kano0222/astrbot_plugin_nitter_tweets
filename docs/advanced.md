...
- `push_targets` 支持在分组详情里新增或删除；点击保存后写回当前分组配置。删除推送目标不会删除关注账号、媒体、推送记录或发送历史。
...
| 配置 | 说明 |
| --- | --- |
| `send_batch_summary_enabled` | 是否发送推送统计与概括摘要（例如“📬 默认分组 · 1 位博主 · 1 条新推文”），默认开启（`true`）。关闭后开启极简纯净模式，全链路（定时、间隔、手动命令）不再发送任何批次横幅、合并统计头节点及推文末尾的 📎 附件统计行，仅保留推文核心内容。 |
...
| 类型 | 传输梯度 |
| --- | --- |
| 图片 | 本地路径 → base64（文件 ≤ `media_transport_base64_max_mb` 时）→ twimg 直链（白名单主机，自动）→ 放弃附件 |
| 视频 | 本地路径 → base64（文件 ≤ `media_transport_base64_max_mb` 时）→ twimg 直链（白名单主机，自动）→ 放弃附件并保留「视频未发送」提示 |

- 视频只有小于 `media_transport_base64_max_mb` 时才会出现 base64 档。默认 8MB 意味着通常不走 base64——
  把几十 MB 的视频塞进单条消息会带来内存和 payload 压力。调高该值即表示接受这一开销。
- twimg 白名单主机的直链会自动加入梯度：本地路径与 base64 都失败后交给协议端直传。这会让协议端
  **直连 Twitter CDN**，绕过插件的代理配置并暴露其出口 IP；网络隔离的部署需自行评估这一暴露。带 token
  的 xdown 直链时效短且对 Referer 敏感，任何时候都不会交给协议端。
- 传输层会记住每个平台最近一次成功的编码档作为下次的起步提示。分容器部署且 AstrBot 与协议端
  不共享文件系统时，本地路径档会失败，自动落到 base64 档。
...
- 默认开启 `send_batch_summary_enabled` 时，后台检查推送的新推文会在本轮每个目标的第一条普通消息或合并转发头部显示批次概览（按类型显示“n 位博主”“n 个搜索订阅”或“n 个 List”、推文数和来源分组）；关闭 `send_batch_summary_enabled` 后进入极简纯净模式：批次横幅、合并转发头部统计节点与推文末尾的 📎 附件统计行全链路均不再发送，仅保留推文核心内容；开启时概括只出现一次，不显示订阅源进度或推文序号。
...
### 5. 推送统计摘要与极简纯净模式（`send_batch_summary_enabled`）

- **功能（Close #74）**：默认开启时，推送前会发送一条推送统计与概括摘要（如“📬 默认分组 · 1 位博主 · 1 条新推文”）。将 `send_batch_summary_enabled` 设置为 `false` 后开启极简纯净模式：全链路（定时、间隔、手动命令）不再发送任何批次横幅与合并统计头节点，推文末尾的 📎 附件统计行也一并省略，仅保留推文核心内容。
- **合并转发兼容**：开启时私人号 OneBot 满足 `merge_tweet_threshold` 走合并转发，概括横幅作为整包的头部索引节点保留；关闭后头部统计节点同样不再发送。
